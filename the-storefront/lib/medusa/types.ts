...
export type Product = Partial<Omit<MedusaProduct, 'tags' | 'options' | 'variants' | 'images'>> & {
    title: string;
...
export type Image = Partial<MedusaImage> & {
    url: string;
    altText: string;
};
...
export type ProductVariant = Partial<MedusaProductVariant> & {
    id: string;
    availableForSale: boolean;
